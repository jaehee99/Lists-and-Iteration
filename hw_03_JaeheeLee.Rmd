...
length_list
names_list
is.vector(names_list)
```

3. The `diploscores` element does not provide any information. Remove it from the list.
```{r}
data[names(data) %in% "diploscores" == FALSE]  -> data  
# check
ls(data)
# Another method
# data[['diploscores']] <- NULL
```
...
scores %>%  
  discard(is.double) %>%  
  map(table) %>%  
  names() -> names_of_var_not_double
names_of_var_not_double
```
...
scores %>%  
  keep(is.double) %>%  
  na.omit() %>%  
  colMeans() -> data$col_means
data$col_means
# original 
# scores %>%  
#  keep(is.double) %>%  
#  na.omit() -> scores_double
# data$col_means <- colMeans(scores_double) 
```
...
data[["col_means"]]
# 4 way 
# list.extract(data, "col_means")

# show to extract the third element of col_means
data[[7]][3]
# show to extract the third element of col_means
data[["col_means"]][3]
```
...
#   }
#   else {
#     return(calcn(x, n-1)+ abs(calcn(x, n-3)- calcn(x, n-2))/4)
#  }
#  }
#}
```
...
tlist <- vector(mode = "list")

for (i in 1:n){
 tlist[[i]] <- t.test(rnorm(10,0,2))
}
...
tlist %>% 
  map(~.$estimate[[1]]) -> list_list

cum_vec <- vector(mode = "double", length = 1000)
for (i in seq_along(cum_vec)) {
    cum_vec[[i]] <- list.extract(list_list, i)
}
...
Null hypothesis: p-value follow the uniform distribution.
Alternative hypothesis: p-value do not follow the uniform distribution. 

t.test shows that p-value is very close to 0. Let's say alpha =0.01, then p-value < alpha. Therefore, we can reject the null hypothesis. So we choose alternative hypothesis which is p-value do not follow the uniform distribution. 

Furthermore,most of the dots are well aligned with the red line. This can indicates that this distribution is a normal distribution. 
