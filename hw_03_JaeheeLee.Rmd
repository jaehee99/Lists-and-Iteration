...
```{r}
# 2 way 
n <- vector(mode = "double", length = 1000)
...
if (n == 1){
  ...
  breaks;
    ...
    return(calcn(n-1)+ abs(calcn(n-3)- calcn(n-2))/4)
      breaks;
...
```{r}
ggplot(data=as.data.frame(qqnorm( pvec_m , plot=F)), mapping=aes(x=x, y=y)) + 
    geom_point() + geom_smooth(method="lm", se=FALSE, color = "red", linetype = "dashed") +ggtitle("QQ PLOT of pvec_m")
```
